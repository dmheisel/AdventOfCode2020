...
class Window:
    ...
    def start(self):
        time.sleep(1)
        self.display(
            'Welcome to the Intcode Network Tester (INT). Any key to continue...')
        ...
        while running:
            ...
            for i, program, computer in cycle(zip(range(5), self.programs, self.network)):
                ...
                for _ in program:
                    ...
                    self.display(
                        f"Computation complete. Final result: {result}. 'q' to quit or any key to continue...")
                    break
                time.sleep(.5)
    ...
    def pre_compute(self, first_time=False):
        ...
        phase_settings = self.ask(
            'Please enter phase setting: ').replace(' ', '')
    ...
    def boot_sequence(self):
        for i in range(5):
            self.network_win.chgat(
                self.top + 6, self.left + 3 + 20 * i, 1, curses.color_pair(3))
            self.network_win.refresh()
            time.sleep(.01)
        ...
        for i in range(5):
            ...
            self.network_win.chgat(
                self.top + 6, self.left + 3 + 20 * i, 1, curses.color_pair(2))
        time.sleep(.5)
        self.network_win.refresh()
